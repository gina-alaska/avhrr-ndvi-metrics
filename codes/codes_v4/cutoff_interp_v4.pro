...
pro cutoff_interp_v4,v_ndvi,  v_bq, v_cldm, v_ndvi_g,  flg_metrics
...
qa_good=15           ; good quality data 
ndvi_low=100         ; valid ndvi is ndvi >=ndvi_low and <=200
cloudy=100           ; >=100, cloudy, <100,clear
...
idx_bad=where(v_ndvi LT ndvi_low or v_bq EQ qa_bad ,cnt_bad)
...
idx_50percent=where( tmpnan-ndvi_low GE 0.2*max(tmpnan-ndvi_low ) and v_bq GE qa_good, cnt50 )
...
;within the range determined by 20% of maximum ndvi value, keep good quality data (greater than 15)
;with cldmask value less than cloudy and ndvi value great than ndvi_low  

idxv=where(tmp50_bq GE qa_good and tmp50 GE ndvi_low  and tmp50_cldm LE cloudy, cnt50v,complement=idxnon);
...
tmpinterp=interpol_line100b_v4(tmpv,tmpx,tmpu)
