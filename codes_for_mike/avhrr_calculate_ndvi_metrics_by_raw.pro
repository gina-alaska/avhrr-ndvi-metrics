...
;
;jzhu, 4/5/2013, this program uses raw time series a input, interpolate, smooth them, then calcualte NDVI metrics

pro avhrr_calculate_ndvi_metrics_by_raw,filen,flg

;flg (indicate if the program run successful, 0--successful, 1--not successful)
;filen-- one-year-stacked raw avhrr file
;

;test only, for simplisity

filen = '/mnt/jzhu_scratch/nps-cesu/avhrr/ak_nd_2009'
...
fileout_metrics=wrkdir+filebasen+'_metrics_by_raw_1_1'
...
;
avhrr_time_series_interp_smooth_process,tmp,bname,vmetrics
...
envi_setup_head, fname=fileout_metrics, ns=ns, nl=nl, nb=nb_metrics,bnames=bnames_metrics, $
    ...
    descrip='avhrr one-year metrics data by raw', /write
