...
;M. Suarez, EROS		Dec, 1998	Modified from C GCTP to IDL
;
;*******************************************************************************/
;#include "cproj.h"
...
;----------------------------------------------------*/
PRO sincos, val, sin_val, cos_val
 sin_val = sin(val)
 cos_val = cos(val)
END
...
;-------------------------------------------*/
FUNCTION phi1z, eccent,qs,flag


      phi = asinz(.5 * qs);
      if (eccent LT EPSLN) THEN $
         return,(phi)
      ...
      FOR i = 1,25 DO BEGIN
        ...
        if (abs(dphi) LE 1e-7) THEN $
          return,phi;

      END
  MESSAGE, "Convergence error in PHI1Z"
  flag = 1;
  return, flag
END
...
FUNCTION phi2z, eccent,ts,flag
  ...
  for i = 0, 15 DO BEGIN
    ...
    sinpi = sin(phi);
    con = eccent * sinpi;
    dphi = !PI/2 - 2 * atan(ts *(( (1.0 - con)/(1.0 + con) )^eccnth)) -$ 
	   phi;
    phi = phi + dphi; 
    if (abs(dphi) LE .0000000001) THEN $
       return,phi;
  END
  MESSAGE,  "Convergence error in PHI2Z";
  flag = 002;
  return, 002
END
...
count = 0;
  ...
  (long(abs(x / (!PI*2))) LT MAXLONG): $
     x = x-((long(x / (!PI*2)))*(!PI*2));

  (long( abs(x / (MAXLONG * (!PI*2)))) LT MAXLONG): $
     x = x-((long(x / (MAXLONG * (!PI*2)))) * ((!PI*2) * MAXLONG));

  (long( abs(x / (DBLLONG * (!PI*2)))) LT MAXLONG): $
     x = x-((long(x / (DBLLONG * (!PI*2)))) * ((!PI*2) * DBLLONG));
...
PRO cproj
; USE .compile cproj to compile all these codes
END
