...
;
;PROGRAMMER              DATE		UPDATE
;----------              ----		------
;D. Steinwand, EROS      March, 1991	Original Development
;M. Suarez, EROS         Dec, 1998	C to IDL
;
...
;  -----------------------------------------------------------------------*/
FUNCTION lamazfor, lon, lat, R=R, lon0=lon0, lat0=lat0, FE=FE, FN=FN
...
if (g EQ -1.0) THEN BEGIN
   print, "Point projects to a circle of radius = ", 2.0 * R
   MESSAGE, "ERROR IN: lamaz-forward"
   return, -1
END
ksp = R * sqrt(2.0 / (1.0 + g));
x = ksp * cos_lat * sin_delta_lon + fe;
