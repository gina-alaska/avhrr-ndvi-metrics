; $Id: cw_bgroup2.pro,v 1.13 1995/04/11 18:12:50 ali Exp $
...
;			font is used. FONT will be used for LABELFONT and/or 
;           BUTTONFONT if either is not specified and FONT is specified.
;	LABELFONT:	The name of the font to be used for the label.
;			If this keyword is not specified, the default
;			font is used.
;	BUTTONFONT:	The name of the font to be used for the buttons.
;			If this keyword is not specified, the default
;			font is used.
...
;	7 January 1997, MGS, Added LABELFONT keyword. Changed name to CW_BGROUP2.
;-
...
function CW_BGROUP2, parent, names, $
	...
	LABELFONT=labelfont, BUTTONFONT=buttonfont, BUTTONSIZE=buttonsize
  ...
  IF (N_ELEMENTS(buttonsize) eq 0)	then buttonsize = 0
  



  IF ((Keyword_Set(font)) AND (NOT Keyword_Set(labelfont))) THEN $
    labelfont=font
  IF ((Keyword_Set(font)) AND (NOT Keyword_Set(buttonfont))) THEN $
    buttonfont=font
  IF (N_Elements(font) EQ 0) THEN $
  	font = ''
  IF (N_Elements(labelfont) EQ 0) THEN $
  	labelfont = ''
  IF (N_Elements(buttonfont) EQ 0) THEN $
  	buttonfont = ''
  top_base = 0L
  if (n_elements(label_top) ne 0) then begin
    next_base = WIDGET_BASE(parent, XOFFSET=xoffset, YOFFSET=yoffset, /COLUMN)
    if(keyword_set(labelfont)) then $
       junk = WIDGET_LABEL(next_base, value=label_top,font=labelfont) $
  ...
  if (n_elements(label_left) ne 0) then begin
    ...
    if(keyword_set(labelfont))then $
       junk = WIDGET_LABEL(next_base, value=label_left, font=labelfont) $
  ...
  ids = lonarr(n)
	for i = 0, n-1 do $
		ids(i) = WIDGET_BUTTON(base, value=names(i), FONT=buttonfont, $
			XSize=buttonSize, UVALUE=i)
  ;	for i = 0, n-1 do begin
  ;	  if (n_elements(buttonfont) eq 0) then begin
  ;	    ids(i) = WIDGET_BUTTON(base, value=names(i), UVALUE=i)
  ;	  endif else begin
  ;	    ids(i) = WIDGET_BUTTON(base, value=names(i), FONT=buttonfont, UVALUE=i)
  ;	  endelse
  ;	endfor
