...
;       mLocal.mInfo = mtmp
   ...
   'View.Smoother': BEGIN 
                    ...
                    END
   'View.Image':    BEGIN
                       IF (NOT Widget_Info(mLocal.wFullView, /Valid_ID)) THEN $
                        mLocal.wFullView =  FullView(mLocal.wBase)
                       Widget_Control, mLocal.wFullView, Get_UValue=mFullView
                       Widget_Control, mFullView.wParent, Get_UValue=mParent
                       mLocal.wOpenFile=mParent.wOpenFile
                    END
   'View.Position': BEGIN
...
;
   'Cube.Smooth Cube':  $
       IF (NOT Widget_Info(mLocal.wCubeSmooth, /Valid_ID)) THEN $
          mLocal.wCubeSmooth = CW_SmoothCube(mLocal)
...
IF (N_Elements(wFullView) EQ 0 ) THEN wFullView = -1L
...
IF (N_Elements(wCoverType) EQ 0 ) THEN wCoverType = -1L
...
Xmanager, 'metrics', wBase, event_handler='metrics2_event',/No_Block
