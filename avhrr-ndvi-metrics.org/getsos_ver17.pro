FUNCTION  GetSOS_ver17, Cross, NDVI, bq, x, bpy, FMA
;
...
;---- compare x20 and possiblex
 ...
 ;------getsos_ver1.pro uses following condition
;         if bq(fix(possibx)) EQ 0b and bq(fix(possibx)+1) EQ 0b and fix(possibx)+1 LE lastidx  then begin ;<4>      

;------getsos_ver17.pro uses following condition

        if fix(possibx) +1 LE lastidx then begin ;<4>
