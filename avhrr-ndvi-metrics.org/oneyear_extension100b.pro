...
;jzhu,9/3/2011, insert randomly 100B to 101B for before and after points

pro oneyear_extension100b,yearv,yearbq,yearbn,st_num,ed_num,days
...
compbq=intarr(st_num+numofyear+ed_num)
compbn=strarr(st_num+numofyear+ed_num)


fillxst=byte( fix( (randomu(1,st_num))*2 )+100 )
fillxed=byte( fix( (randomu(1,ed_num))*2 )+100 )

 compv[0:st_num-1]=fillxst
compbq[0:st_num-1]=20b

 compv[st_num:st_num+numofyear-1]=yearv
compbq[st_num:st_num+numofyear-1]=yearbq

 compv[st_num+numofyear: st_num+numofyear+ed_num-1]=fillxed
compbq[st_num+numofyear: st_num+numofyear+ed_num-1]=20b
